$(function () {
    ...
    $.ajax({//获取商品列表
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.data.length; i++) {
                ...
                var nameM = nameS / 100;
                clats += "<li class='one-goods' lay-tt='" + data.data[i].shopGoodsCode + "'><div class='delete' style='width: 13%;line-height: 100px;color: red;text-align: center;'>删除</div><input type='checkbox' class='GoodsCheck' style='display: none'> <div style='width: 32%;' class='lists'><img src='" + data.data[i].actGoods.imgUrl + "'><p class='smoke'><b style='width:69px;overflow: hidden;text-overflow: ellipsis;'>" + data.data[i].actGoods.name + "</b><b>￥<span class='GoodsPrice'>" +nameM+ "</span>元/"+data.data[i].unit+"</b></p></div><div style='width: 26%;' class='list'><a href='javascript:;' class='reduce'><input type='button' value='-'></a><span><input type='text' value='1' class='number' readonly='readonly'></span><a href='javascript:;' class='plus'><input type='button' value='+'></a></div><div style='margin-top:35px;white-space: nowrap;width: 28%;overflow: hidden;box-sizing: border-box;padding-left:6px;'>总计:<br/>&nbsp ￥<b class='single'>" + nameM + "元</b></div></li>"
            }
    ...
    function call(payWay) {
       ...
       // var openId = memberIdL.split('?')[1];
        ...
        var customerPhone = localStorage.getItem("customerPhone");//新加字段 电话
        var customerName = localStorage.getItem("customerName");//新加字段 姓名
       
        var workNo = localStorage.getItem("workNo");// 店员id
