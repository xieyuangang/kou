...
},beforePageText:"第",afterPageText:"页  共{pages}页",displayMsg:"当前显示 {from} 至 {to} 条记录   共 {total} 条记录",nav:{first:{iconCls:"pagination-first",handler:function(){
...
if(_24f.mask){
_24f.mask.css({display:"block",zIndex:"1"});
}
if(_24f.shadow){
_24f.shadow.css({display:"block",zIndex:"1",left:_24f.options.left,top:_24f.options.top,width:_24f.window._outerWidth(),height:_24f.window._outerHeight()});
}
_24f.window.css("z-index","2");
...
if(_256.mask){
_256.mask.css("z-index","1");
}
if(_256.shadow){
_256.shadow.css("z-index","1");
}
_256.window.css("z-index","2");
if(!_256.proxy){
...
if(opts.pagination){
$.extend(_646,{pageNumber:opts.pageNumber,pageSize:opts.pageSize});
}
if(opts.sortName){
...
if(opts.pagination){
$.extend(_7ae,{pageNumber:opts.pageNumber,rows:opts.pageSize});
}
if(opts.sortName){
...
$(function(){
	$(".window-mask").height($(window).height()).width($(window).width());
});
