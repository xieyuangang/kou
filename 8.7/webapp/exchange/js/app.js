var myApp = angular.module('myApp',['ui.router'],function($httpProvider) {	//初始化
    // 头部配置，http请求配置
...
}).filter('handleExchangeDate' , function(){
	return function(value){
		var s = value.split(" ");
		return s[0];
	}
...
}).filter('cutCreateTime' , function(){//统计日期拆分字符串
	return function(value){
		var a = value.split(" ")[0];
		var b = value.split(" ")[1];
		var c = a.split("-");
		//var d=b.split(":")
		return c[0]+"年"+c[1]+"月"+c[2]+"日 "+b;//[1]+"点"+b[3]+b[4]+"分"
	}
});
...
myApp.service("tool" , function(){
	...
	this.refreshIscroll = function(id , onRefresh , onScrollMove , onScrollEnd){
		this.iScroll = new iScroll(id, { 
			vScrollbar:false,
			useTransition: true,
			onRefresh : onRefresh,
